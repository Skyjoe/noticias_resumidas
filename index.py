...
# Usando cache em memória com limite de tamanho para evitar consumo excessivo
cache = Cache(app, config={
    'CACHE_TYPE': 'simple',
    'CACHE_DEFAULT_TIMEOUT': 300,
    'CACHE_THRESHOLD': 100  # Limite de itens no cache
})
...
class NewsManager:
    ...
    def get_news(self, query, start=0, count=3):
        ...
        """
        with self.lock:
            # Verifica se já fez requisição recente para mesma query
            current_time = time.time()
            last_time = self.last_request.get(query, 0)
            
            # Espera mínima de 2 segundos entre requisições para mesma query
...
def get_news():
    ...
    # Limita quantidade máxima de resultados para evitar sobrecarga
    count = min(count, 20)
